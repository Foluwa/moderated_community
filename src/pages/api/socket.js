...
export default (async (req, res) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("login", async (data) => {
      ...
      if (userSockets.length > 0) return socket.emit("login", { error: "Username is already loggedin" });
    ...
    socket.on("createRoom", data => {
      ...
      if (io.sockets.adapter.rooms[name]) return socket.emit("createRoom", { success: false, error: "This room already exists" });
      let room = {
    ...
    socket.on("joinRoom", async data => {
      ...
      if (!io.sockets.adapter.rooms[id]) return socket.emit("joinRoom", { success: false, error: "This room not found" });

      const room = io.sockets.adapter.rooms[id];
      if (room.password && room.password !== password) return socket.emit("joinRoom", { success: false, error: "You typed the wrong password" });
